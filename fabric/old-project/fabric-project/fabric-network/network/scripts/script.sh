...
function instantiateChaincode(){
  ...
  CMD='{"Args":["init","'${TZ}'"]}'
  peer chaincode instantiate -o ${CONNECT_ORDERER} --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} -l ${LANGUAGE} -v 1.0 -c ${CMD} -P "AND ('Org1MSP.peer')" >&log.txt
  res=$?
  cat log.txt
  if [ $res -ne 0 ]; then
