...
var enrollAdmin = async function (values) {
    try {
        ...
        if (adminExists) {
            console.log(`An identity for the admin user ${values.enrolled} already exists in the wallet`);
            return;
        }
        ...
        console.log(`Successfully enrolled admin user ${values.enrolled} and imported it into the wallet`);
...
var registerUser = async function(values) {
    try {
        ...
        const enrollment = await ca.enroll({ enrollmentID: values.newuser, enrollmentSecret: secret, profile: 'tls' });
        ...
        console.log(`Successfully registered and enrolled admin user ${values.newuser} and imported it into the wallet`);

    } catch (error) {
        console.error(`Failed to register user ${values.newuser}: ${error}`);
        process.exit(1);
    }
}
